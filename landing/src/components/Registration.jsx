...
export default function Registration() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password.length < 12) {
      alert("Use at least 12 characters for your password.");
      return;
    }
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8787"}/api/auth/register`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ companyName: formData.company, displayName: `${formData.firstName} ${formData.lastName}`.trim(), email: formData.email, password: formData.password }) });
      if (!response.ok) { const payload = await response.json().catch(() => ({})); alert(payload.error?.message || "Registration failed."); return; }
      const payload = await response.json();
      navigate("/app");
    } catch { alert("Breeze is temporarily unavailable. Please try again."); }
  };
            ...
            <div className="rounded-2xl bg-paper p-4">
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span className="text-sm leading-6 text-muted">
                  I agree to the Breeze terms of service and privacy policy.
                </span>
              </label>
            </div>
